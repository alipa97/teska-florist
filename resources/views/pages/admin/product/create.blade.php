...
@section('title', 'Tambah Produk | Teska')

@section('content')
<section class="max-w-screen-xl px-4 py-8 mx-auto sm:px-6 lg:px-8">
  <div class="p-6 bg-white rounded-lg shadow-lg">
    <h2 class="mb-6 text-2xl font-semibold text-gray-800">Tambah Produk Baru</h2>
    ...
    <form action="{{ route('dashboard.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
      ...
      <div>
        ...
        <input type="text" name="name" id="name" class="w-full px-4 py-2 text-sm border rounded-lg focus:ring focus:ring-pink-200 focus:outline-none" placeholder="Masukkan nama produk" value="{{ old('name') }}" required>
        @error('name')
      ...
      <div>
        ...
        <input type="number" name="price" id="price" class="w-full px-4 py-2 text-sm border rounded-lg focus:ring focus:ring-pink-200 focus:outline-none" placeholder="Masukkan harga produk" value="{{ old('price') }}" required>
        @error('price')
        <span class="text-sm text-red-500">{{ $message }}</span>
        @enderror
      </div>
      ...
      <div>
        ...
        <input type="number" name="stock_quantity" id="stock_quantity" class="w-full px-4 py-2 text-sm border rounded-lg focus:ring focus:ring-pink-200 focus:outline-none" placeholder="Masukkan jumlah stok" value="{{ old('stock_quantity') }}" required>
