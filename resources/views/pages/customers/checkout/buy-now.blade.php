...
@section('title', 'Pembayaran | Teska')

@section('content')

<div class="bg-white">
  <div class="fixed top-0 right-0 hidden w-1/2 h-full bg-white lg:block" aria-hidden="true"></div>
  <div class="fixed top-0 left-0 hidden w-1/2 h-full bg-gray-100 lg:block" aria-hidden="true"></div>
  ...
  <main class="relative grid grid-cols-1 mx-auto max-w-7xl gap-x-16 lg:grid-cols-2 lg:px-8 xl:gap-x-48">
    ...
    <section aria-labelledby="summary-heading" class="px-4 pt-16 pb-36 sm:px-6 lg:col-start-1 lg:row-start-1 lg:px-0 lg:pb-16">
      <div class="max-w-lg mx-auto lg:max-w-none">
        ...
        <ul role="list" class="text-sm font-medium text-gray-900 divide-y divide-gray-200">
          <li class="flex items-start py-6 space-x-4">
            <img src="{{ asset('./storage/products/' .$product->image) }}" alt="{{ $product->name }}" class="flex-none object-cover object-center w-20 h-20 rounded-md">
    ...
    <form action="{{ route('checkout.buy_now_store', $product->slug) }}" method="POST" enctype="multipart/form-data" class="px-4 pt-16 pb-10 bg-gray-50 sm:px-6 lg:row-start-1 lg:bg-transparent lg:px-0 lg:pb-16">
      @csrf
      <div class="max-w-lg mx-auto lg:max-w-none">
        <section aria-labelledby="contact-info-heading">
          ...
          <div class="mt-6">
            <label for="order_name" class="block text-sm font-medium text-gray-700">Nama Pemesan</label>
            <div class="mt-1">
              <input type="text" id="order_name" name="order_name" class="block w-full h-12 px-4 bg-white border border-gray-400 shadow-sm focus:border-gray-500 focus:ring-gray-500 sm:text-sm" required maxlength="255">
            </div>
          </div>
          
          <div class="mt-6">
            <label for="order_phone" class="block text-sm font-medium text-gray-700">No. Telepon</label>
            <div class="mt-1">
              <input type="text" id="order_phone" name="order_phone" class="block w-full h-12 px-4 bg-white border border-gray-400 shadow-sm focus:border-gray-500 focus:ring-gray-500 sm:text-sm" required pattern="\d+">
            </div>
          </div>
        </section>
